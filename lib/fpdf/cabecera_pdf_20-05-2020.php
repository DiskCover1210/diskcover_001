...
class cabecera_pdf
{
	...
	function cabecera_reporte($titulo,$tablaHTML,$contenido=false,$image=false,$fechaini,$fechafin,$sizetable,$mostrar=false,$sal_hea_body)
	{	
 ...
 function cabecera_reporte_MC($titulo,$tablaHTML,$contenido=false,$image=false,$fechaini,$fechafin,$sizetable,$mostrar=false,$sal_hea_body)
	{	
		...
		if($contenido)
		{
		 foreach ($contenido as $key => $value) {
		    ...
		    foreach ($tablaHTML as $key => $value){
		    	...
		    	if(isset($value['borde']) && $value['borde']!='')
		    	{
		    		$borde=1;
		    	}else
		    	{
