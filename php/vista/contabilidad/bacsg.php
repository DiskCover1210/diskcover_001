...
<script type="text/javascript">
  ...
  $(document).ready(function()
  {
	...
	{
		var opcion = '<option value="00">Seleccione</option>';
		var s = '<?php if(isset($_GET['tip'])){ echo $_GET['tip']; } ?>';
        ...
        success:  function (response) { 
        	...
        	$.each(response,function(i,item){
        		if(s !='' && s == item.Codigo)
        		{
        			 ...
        			 $('#optionsCheck1').prop('checked',true);
        			opcion+= '<option value="'+item.Codigo+'" selected>'+item.Detalle+'</option>'
        		}else{
        		opcion+= '<option value="'+item.Codigo+'">'+item.Detalle+'</option>'
        	 }
        	})
        $('#tipo_ba').html(opcion);
      }
	...
	{
		var tip=$('#tipo_ba').val(); 
	$('#l1').attr('href', "contabilidad.php?mod=contabilidad&acc=bacsg&acc1=Balance de Comprobacion/Situación/General&                        ti=BALANCE DE COMPROBACIÓN&Opcb=1&Opcen=0&b=1&bm=0&fechai=<?php echo date('Y-m-d',strtotime($_SESSION['INGRESO']['Fechai'])) ?>
                        &fechaf=<?php echo date('Y-m-d',strtotime($_SESSION['INGRESO']['Fechaf'])) ?>&tip="+tip+'&OpcCE=0');
	}
</script>
             ...
             <div class="box" style='margin-bottom: 5px;'>
              <div class="box-header">
                    <h4 class="box-title">
                        ...
                        &fechaf=<?php echo date('Y-m-d',strtotime($_SESSION['INGRESO']['Fechaf'])) ?>&tip=<?php echo $tip; ?>">
...
<?php
    ...
    if(isset($_GET['bm'])) 
    {
        ...
        else
        {
            ...
            sp_Procesar_Balance($_SESSION['INGRESO']['Opc'],$_SESSION['INGRESO']['Sucursal'],$_SESSION['INGRESO']['item'],
            $_SESSION['INGRESO']['periodo'],$_GET['fechai'],$_GET['fechaf'],$_GET['bm'],$_GET['tip']);
        }
                ...
                <div class="row">
                    ...
                    <div class="col-sm-2">
                        <div class="input-group">
                              ...
                              <input type="date" class="form-control pull-right input-sm" id="desde" placeholder="01/01/2019" name="fechai" onKeyPress="return soloNumeros(event)"  maxlength="10"  onchange='modificar("NI");' value='<?php echo date('Y-m-d',strtotime($_SESSION['INGRESO']['Fechai'])) ?>' onkeyup="validar_year_mayor(this.id)" onblur="validar_year_menor(this.id)">
                        </div>
                    ...
                    <div class="col-sm-2">
                        <div class="input-group">
                            ...
                            value='<?php echo date('Y-m-d',strtotime($_SESSION['INGRESO']['Fechaf'])) ?>' onkeyup="validar_year_mayor(this.id)" onblur="validar_year_menor(this.id)">
                        </div>
                    </div>
                    <div class="col-sm-3">
                        <div class="row">
                        ...
                        ?>
                          ...
                          <?php echo $check; ?> onclick='modificarb("optionsCheck1");'>                       
                            ...
                            <div class="col-sm-7" id="cc" style="display: none">
                                <select class="form-control" id="tipo_ba" onchange="selec_tip()">
...
<script>
    ...
    function modificarb(id){
        ...
        {
            $('#cc').css('display','none');
            $('#l1').attr('href', "contabilidad.php?mod=contabilidad&acc=bacsg&acc1=Balance de Comprobacion/Situación/General&                        ti=BALANCE DE COMPROBACIÓN&Opcb=1&Opcen=0&b=1&bm=0&fechai=<?php echo date('Y-m-d',strtotime($_SESSION['INGRESO']['Fechai'])) ?>&fechaf=<?php echo date('Y-m-d',strtotime($_SESSION['INGRESO']['Fechaf'])) ?>&OpcCE=0");
