<?php
/*$arr = array(2,3,4,3,2,1);
$ent=3;
echo "el numero: ".$ent.' se repite: '.buscarnum($arr,$ent);
function buscarnum($arr,$valor)
{
	if(!is_array($arr))
	{		
		return NULL;
	...
	return array_key_exists($valor,$arr)?$v[$valor]:0;
}
die();

mauro es un desarrollador de python que ha estado trabajando para cubicle inc. 
durante 8 meses en un proyecto de aumento de personal. vive en campinhas, cerca de sao paulo. su desempeño en el proyecto ha sido con altibajos,
y su comunicación ha sido errática, a veces resultó ser difícil de encontrar. nunca lograste crear un vínculo fuerte con mauro 
durante los 8 meses que ha estado en el proyecto, ese es también el período en el que ha estado trabajando para bairesdev.

el martes recibió un correo electrónico de stefan (propietario del producto en cubicle inc.) que indicaba que no 
 contactar a mauro durante los últimos dos días, y que estaba particularmente preocupado ya que tuvieron un despliegue crítico el viernes y 
 confiaron mucho en mauro para.
 
Intentaste llegar a Mauro por Skype, pero su estado indicaba que estaba desconectado. Usted le escribió un correo electrónico solicitando 
contactarlo lo antes posible. Un par de horas más tarde, recibió una respuesta de Mauro que indica brevemente.

mi esposa tiene un automóvil accidental durante el fin de semana y está en el hospital en este momento, no estoy seguro de cuánto tiempo. 
Intentaré trabajar tanto como pueda. Me pondré en contacto directamente con Stefan para informarle sobre la situación.

Para determinar si continúa sin conexión en Skype, y siendo el correo electrónico la única forma de llamar su atención, 
escriba el texto completo del correo electrónico (en inglés) que le escribirá a Mauro como respuesta.
También puede incluir una explicación más allá del correo electrónico (no debe enviarse a Mauro), explicando por qué escribió en el cuerpo del correo 
electrónico, como parte de este mismo ejercicio. diferencie claramente esta explicación del cuerpo del correo electrónico a mauro
...
if( file_exists("entidad".$entidad) == true )
{
